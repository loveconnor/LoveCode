import { PENPAL_CHILD_CHANNEL, type PromisifiedPenpalParentMethods } from '@fluxly/penpal';
...
/**
 * Find the correct parent window for Fluxly connection.
 * Handles both direct iframes (Next.js) and nested iframes (Storybook).
 */
const findFluxlyParent = (): Window => {
    ...
    // This is the Next.js case: Fluxly -> Next.js iframe
    if (window.parent === window.top) {
        return window.parent;
    }

    // We're in a nested iframe (parent is NOT the top window)
    // This is the Storybook case: Fluxly -> CodeSandbox -> Storybook preview iframe
    if (window.top) {
