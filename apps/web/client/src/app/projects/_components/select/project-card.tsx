...
import { STORAGE_BUCKETS } from '@fluxly/constants';
import type { Project } from '@fluxly/models';
import { timeAgo } from '@fluxly/utility';
        ...
        >
            <div className={`relative ${aspectRatio} rounded-lg overflow-hidden shadow-sm hover:shadow-xl hover:shadow-black/20 transition-all duration-300 group`}>
                {img ? (
                    ...
                    <>
                        <div className="absolute inset-0 w-full h-full bg-gradient-to-t from-gray-900 via-gray-800 to-gray-900" />
                        <div className="absolute inset-0 flex items-center justify-center opacity-20">
                            <Icons.FluxlyLogo className="w-20 h-20 text-white" />
                        </div>
                        <div className="absolute inset-0 rounded-lg border-[0.5px] border-white/10" />
                    </>
                )}
