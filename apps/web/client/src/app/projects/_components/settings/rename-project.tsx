...
import type { Project } from '@fluxly/models';
import {
    Dialog,
    DialogContent,
    DialogFooter,
    DialogHeader,
    DialogTitle,
} from '@fluxly/ui/dialog';
import { Button } from '@fluxly/ui/button';
import { DropdownMenuItem } from '@fluxly/ui/dropdown-menu';
import { Icons } from '@fluxly/ui/icons';
import { Input } from '@fluxly/ui/input';
import { Label } from '@fluxly/ui/label';
...
export function RenameProject({ project, refetch }: { project: Project; refetch: () => void }) {
    ...
    const handleRenameProject = async () => {
        ...
        window.dispatchEvent(new CustomEvent('fluxly_project_updated', {
            detail: {
                id: project.id,
                name: projectName,
                metadata: {
        ...
        window.dispatchEvent(new CustomEvent('fluxly_project_modified', { detail: { id: project.id } }));
    ...
    return (
        <>
            <DropdownMenuItem
                ...
                className="text-foreground-active hover:!bg-background-fluxly hover:!text-foreground-active gap-2"
            >
