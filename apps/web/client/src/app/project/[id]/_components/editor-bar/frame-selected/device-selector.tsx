...
import { DEVICE_OPTIONS, Orientation } from '@fluxly/constants';
import type { WindowMetadata } from '@fluxly/models';
import { Icons } from '@fluxly/ui/icons/index';
import { Select, SelectContent, SelectGroup, SelectItem, SelectLabel, SelectSeparator, SelectTrigger } from '@fluxly/ui/select';
import { cn } from '@fluxly/ui/utils';
import { computeWindowMetadata, getDeviceType } from '@fluxly/utility';
                        ...
                        {Object.entries(devices).map(([name, dimensions]) => (
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <DeviceIcon deviceType={category} orientation={metadata.orientation} className={`${device === `${category}:${name}` ? 'text-foreground-primary' : 'text-foreground-fluxly'}`} />
                                    <span className="text-xs">{name}</span>
                                    <span className={`text-xs ml-auto ${device === `${category}:${name}` ? 'text-foreground-primary' : 'text-foreground-tertiary'}`}>{dimensions.replace('x', '×')}</span>
                                </div>
                            </SelectItem>
                        ))}
                        <SelectSeparator />
