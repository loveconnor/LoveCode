...
import { useFile } from '@fluxly/file-system/hooks';
import type { ImageContentData } from '@fluxly/models';
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
import {
    ...
    DropdownMenuItem,
    DropdownMenuTrigger
} from '@fluxly/ui/dropdown-menu';
import { Icons } from '@fluxly/ui/icons';
import { Input } from '@fluxly/ui/input';
import { getMimeType, isVideoFile } from '@fluxly/utility';
...
export const ImageItem = ({ image, projectId, branchId, onImageDragStart, onImageDragEnd, onImageMouseDown, onImageMouseUp, onRename, onDelete, onAddToChat }: ImageItemProps) => {
    ...
    return (
        <div className="group">
            <div
                className="aspect-square bg-background-secondary rounded-md border border-border-primary overflow-hidden cursor-pointer hover:border-border-fluxly transition-colors relative"
                        ...
                        <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
                            <DropdownMenuTrigger asChild>
                                <Button
                                    size="icon"
                                    variant="secondary"
                                    className="h-6 w-6 bg-background-secondary/90 hover:bg-background-fluxly"
                                    onClick={(e) => {
                                        e.preventDefault();
                                        e.stopPropagation();
                                    }}
