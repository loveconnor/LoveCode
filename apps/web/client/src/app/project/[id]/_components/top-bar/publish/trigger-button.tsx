...
import { DeploymentStatus, DeploymentType } from '@fluxly/models';
import { Button } from '@fluxly/ui/button';
import { DropdownMenuTrigger } from '@fluxly/ui/dropdown-menu';
...
export const TriggerButton = observer(() => {
    ...
    let colorClasses = 'border-input bg-background hover:bg-background-fluxly text-foreground-primary';
    ...
    if (isCompleted) {
        ...
        colorClasses = 'border-input bg-background hover:bg-background-fluxly text-foreground-primary hover:border-foreground-primary';
    }
    ...
    return (
        <DropdownMenuTrigger asChild>
            <Button
                ...
                size="sm"
                className={cn(
                    'px-3 flex items-center text-xs justify-center h-8 rounded-md transition-all duration-300 ease-in-out',
                    colorClasses,
                )}
            >
                {icon}
