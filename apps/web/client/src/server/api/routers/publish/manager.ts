...
} from '@fluxly/constants';
import type { Deployment, deploymentUpdateSchema } from '@fluxly/db';
import { addBuiltWithScript, injectBuiltWithScript } from '@fluxly/growth';
import { DeploymentStatus } from '@fluxly/models';
import { addNextBuildConfig } from '@fluxly/parser';
import {
    convertToBase64,
    isBinaryFile,
    isEmptyString,
    ...
    async publish({
        ...
        if (!skipBadge) {
            await updateDeployment({
                id: deploymentId,
                status: DeploymentStatus.IN_PROGRESS,
                message: 'Adding "Built with Fluxly" badge...',
                progress: 35,
                envVars,
            });
            await this.addBadge('./');
