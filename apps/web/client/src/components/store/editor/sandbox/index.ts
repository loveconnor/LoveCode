...
import { EXCLUDED_SYNC_PATHS } from '@fluxly/constants';
import type { CodeFileSystem } from '@fluxly/file-system';
import { type FileEntry } from '@fluxly/file-system';
import type { Branch, RouterConfig } from '@fluxly/models';
    ...
    async downloadFiles(
        ...
        try {
            ...
            return {
                ...
                // to handle this case
                downloadUrl: url ?? '',
                fileName: `${projectName ?? 'fluxly-project'}-${Date.now()}.zip`,
            };
        } catch (error) {
