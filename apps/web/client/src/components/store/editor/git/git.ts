...
import { SUPPORT_EMAIL } from '@fluxly/constants';
import { type GitCommit } from '@fluxly/git';

import type { SandboxManager } from '../sandbox';
import { prepareCommitMessage, sanitizeCommitMessage, withSyncPaused } from '@/utils/git';

export const ONLOOK_DISPLAY_NAME_NOTE_REF = 'refs/notes/fluxly-display-name';
...
export class GitManager {
    ...
    async ensureGitConfig(): Promise<boolean> {
        try {
            ...
            if (!hasName) {
                const nameConfigResult = await this.runCommand('git config user.name "Fluxly"');
     ...
     */
    async createCommit(message = 'New Fluxly backup'): Promise<GitCommandResult> {
