...
import { DefaultSettings, EditorAttributes } from '@fluxly/constants';
import type {
    DomElement,
...
import { EditorMode, InsertMode } from '@fluxly/models';
import {
    type ActionElement,
    type ActionLocation,
    type ActionTarget,
...
} from '@fluxly/models/actions';
import { StyleChangeType } from '@fluxly/models/style';
import { colors } from '@fluxly/ui/tokens';
import { canHaveBackgroundImage, createDomId, createOid, urlToRelativePath } from '@fluxly/utility';
...
export class InsertManager {
    ...
    async end(e: React.MouseEvent<HTMLDivElement>, frameView: IFrameView | null) {
        ...
        if (!mode) {
            console.error('No insert mode set');
            return;
        }

        const origin = getRelativeMousePositionToFrameView(e, frameView);
        await this.insertElement(frameView, newRect, origin, mode);
        this.drawOrigin = undefined;
        this.editorEngine.state.editorMode = EditorMode.DESIGN;
    }

    async drop(e: React.DragEvent<HTMLDivElement>, frameView: IFrameView | null, mode: InsertMode) {
        if (!frameView) {
            console.error('frameView not found');
            return;
        }
        ...
        if (mode === InsertMode.INSERT_TEXT) {
            width = 200;
            height = 30;
        }

        const newRect: RectDimensions = {
            top: 0,
            left: 0,
            width,
            height,
        ...
        const success = await this.insertElement(frameView, newRect, origin, mode);
        if (!success) {
    ...
    async insertElement(frameView: IFrameView, newRect: RectDimensions, origin: ElementPosition, mode: InsertMode) {
        const insertAction = await this.createInsertAction(frameView, newRect, origin, mode);
        if (!insertAction) {
    ...
    async createInsertAction(
        ...
        // Ensure target oid exists for code generation; fallback to action element lookup
        if (!location.targetOid) {
            const targetElement = await frameView.getActionElement(location.targetDomId);
            if (targetElement?.oid) {
                location.targetOid = targetElement.oid;
            } else {
                console.error('Insert target missing oid');
                return;
            }
        }
        ...
        const actionElement: ActionElement = {
            ...
            oid,
            branchId,
            tagName: mode === InsertMode.INSERT_TEXT ? 'p' : mode === InsertMode.INSERT_IMAGE ? 'img' : 'div',
            attributes: {
                [EditorAttributes.DATA_ONLOOK_DOM_ID]: domId,
                [EditorAttributes.DATA_ONLOOK_INSERTED]: 'true',
                [EditorAttributes.DATA_ONLOOK_ID]: oid,
                ...(mode === InsertMode.INSERT_IMAGE ? { src: 'https://picsum.photos/200/300' } : {}),
            },
            children: [],
            textContent: mode === InsertMode.INSERT_TEXT ? 'Enter text...' : null,
            styles,
        };
