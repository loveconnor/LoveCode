...
Codesandbox may ask you to confirm to proceed. You can click the button by switching to Preview mode in fluxly, and click the "Yes, proceed to preview" link and switch back. The UI should render properly inside the iframe after that.
...
If you want to update the preload script and test it with Fluxly, follow these steps:

1. Run bun `bun run dev` from the root of the project.
2. Copy the file `apps/web/client/public/fluxly-preload-script.js` into to `public/fluxly-preload-script.js` in the code tab in Fluxly or the codesandbox interface.
