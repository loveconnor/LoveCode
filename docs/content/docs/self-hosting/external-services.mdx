...
description: External services for Fluxly
---

# External Services

Fluxly requires several external services to provide its features. Each of these services is self-hostable under Apache 2.0 or equivalently permissive licenses.

## Self-hosting External Services

For assistance with self-hosting these services, please contact us at [founders@fluxly.com](mailto:founders@fluxly.com).
...
1. Update the providers in [`packages/ai/src/chat/providers.ts`](https://github.com/fluxly-dev/fluxly/blob/main/packages/ai/src/chat/providers.ts). We already support Anthropic and OpenRouter as examples. You can follow the same format to add a new provider.
2. Update the usages by searching for [`initModel`](https://github.com/search?q=repo%3Afluxly-dev%2Ffluxly+%22await+initModel%22&type=code)
