...
Server-side image processing utilities for Fluxly using Sharp.
...
This package is part of the Fluxly monorepo and should be added as a dependency to packages that need server-side image processing.
...
```typescript
import { compressImageServer } from '@fluxly/image-server';
...
```typescript
import { batchCompressImagesServer } from '@fluxly/image-server';
...
```typescript
import { compressImageServer } from '@fluxly/image-server';
import { COMPRESSION_IMAGE_PRESETS } from '@fluxly/constants';
